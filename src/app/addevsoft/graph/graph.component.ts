...
export class GraphComponent {
  ...
  ngAfterViewInit() {
    this.http.get('https://canvasjs.com/data/gallery/angular/btcusd2021.json', { responseType: 'json' }).subscribe((response: any) => {
      ...
      this.months = Number(this.maxValue.x - this.minValue.x);
      this.daysPassed = Math.floor(this.months / (1000 * 60 * 60 * 24));
      this.monthsPassed = Math.floor(this.daysPassed / 30);
      this.yearsPassed = Math.floor(this.monthsPassed / 12);
